...
Перед первым запросом ввода должен быть выведен аргумент start_message, после каждого ошибочного ввода нужно выводить значение 
строки error_message и при удачном вводе нужно вывести строку end_message и вернуть полученное целое число из функции (см. пример работы). 
...
def get_int(start_message, error_message, end_message):
    print(start_message)   #выводим первый аргумент
    i = 0
    while i >= 0:
        x = input()   #считываем строку
        try:   #пытаемся
            x = int(x)   #привести её к типу int
        except:    #если не получается, то
            print(error_message)   #выводим строку-ошибку
            continue   #продолжаем цикл
        print(end_message)  #иначе выводим строку-завершение
        break    #завершаем цикл
    return x   #возвращаем полученное целое число
  
x = get_int('Input int number:', 'Wrong value. Input int number:', 'Thank you.')   #пример вызова функции с аргументами
